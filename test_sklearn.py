...
pipelines = {
    (s, m): make_pipeline(scalers[s], models[m]).fit(X_train, y_train) 
        for m in models 
        for s in scalers
    }
...
t.add_rows([
    [
        key[1], 
        key[0], 
        f"{scorers['mse'](pipeline, X_test, y_test):.4f}", 
        f"{scorers['r2'](pipeline, X_test, y_test):.4f}"
...
def PCA_shower(n_components, preprocessor):
    ...
    t = PrettyTable(
        ["Feature"] + \
        [f"Component {i+1}" for i in range(len(pca.components_))]
    )
    t.add_row(
    ...
    t.add_rows([[feature] + [f"{pca.components_[j][i]:.4f}"
                for j in range(len(pca.components_))]
                for i, feature in enumerate(ultimate_data_dict.keys())])
    return t
...
tree_scores=[
    (i, mean_squared_error(y_test, tree.predict(X_test)))
...
plot_tree(best_tree, max_depth=2, feature_names=[*Z.keys()], filled=True)
plt.show()