...
date_converter = lambda item: \
    None if item == None or item == '' \
    else pot_dates(date(*[
        int(i) for i in (
            item[0:4],
            item[5:7],
...
cur = con.cursor()
gen = lambda col: cur.execute(
    f"SELECT {col} FROM people WHERE is_recid != -1"
)
...
none_number = lambda caller, data, columns: \
print(*[
    f"{caller}" + \
    f"{i}: " + \
    f"Nones: {data[i].count(None)} " + \
    f"Uniques: {len(set(data[i]))} " + \
    f"Total: {len(data[i])}"
    for i in columns
], "", sep="\n")
...
# none_number('projected_data: ', short_data, short_columns)
...
for i in range(len(race_uniques)):
    short_data[race_list[i]] = \
    [int(item == race_uniques[i]) for item in short_data['race']]
# revised columns
# none_number('transformed_data: ', short_data, transformed)
kys = [*short_data.keys()]
#print(kys)
kys.remove('sex')
kys.remove('race')
